...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className="fixed top-0 left-0 right-0 z-50 px-8 nav-glass"
      ...
      >
        <div className="max-w-[1600px] mx-auto flex justify-between items-start">
          ...
          <div ref={signatureRef} className="flex flex-col gap-1">
            <a href="#home" className="text-[0.95rem] font-semibold tracking-[0.08em] text-white hover:text-white/80 transition-colors">
              {t.profile.title}
            </a>
          ...
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
            {LANG_ORDER.map((code) => (
              <button
                ...
                className={`relative flex h-8 min-w-[2.25rem] items-center justify-center rounded-full bg-[#c92a2a] text-[0.7rem] font-semibold tracking-[0.28em] uppercase text-white transition-all duration-300 ${
                  language === code
              ...
              >
                <span className="px-3 drop-shadow-[0_0_6px_rgba(255,255,255,0.45)]">{t.languageNames[code]}</span>
              </button>
            ))}
          </div>
      ...
      <div className="fixed top-[70px] left-8 z-40 flex gap-1">
        {t.profile.year.split('').map((digit, index) => (
          <span
            key={`${digit}-${index}`}
            className="flex h-7 w-7 items-center justify-center rounded-full bg-[#c92a2a] text-[0.75rem] font-semibold text-white shadow-[0_0_12px_rgba(201,42,42,0.35)]"
          >
            {digit}
          </span>
        ))}
